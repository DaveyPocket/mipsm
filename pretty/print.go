...
func PrintJType(t_opcode uint8, t_imm uint32) string {
	return fmt.Sprintf("x\"%x%x\",x\"%x%x\",x\"%x%x\",x\"%x%x\", \n", (t_opcode&0x3C)>>2, (t_opcode&3)<<2+uint8(((t_imm&0x0F000000)>>24)&2), (t_imm&0x00F00000)>>20, (t_imm&0x000F0000)>>16, (t_imm&0x0000F000)>>12, (t_imm&0x00000F00)>>8, (t_imm&0x000000F0)>>4, (t_imm & 0x0000000F))
}
